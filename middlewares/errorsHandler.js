const errorsHandler = (err, req, res, next) => {
  switch (err.message) {
    ...
    case "jwt expired":
      return res.status(401).json({
        error: { code: "token/token-expired", message: "Token expired" },
      });
    ...
    case "invalid token":
      return res.status(401).json({
        error: { code: "token/invalid-token", message: "Invalid token" },
      });

    case "Token not found":
      return res.status(401).json({
        error: { code: "token/token-not-found", message: "Token not found" },
      });
    ...
    case "Tasks not found":
      return res.status(404).json({
        error: { code: "tasks/tasks-not-found", message: "Tasks not found" },
      });

    case "Task not found":
      return res.status(404).json({
        error: { code: "tasks/task-not-found", message: "Task not found" },
      });
